...
const Cart = () => {
    const { cart, addToCartByQuantity, removeFromCart } = useContext(CartWishlistContext);

    const handleQuantityChange = (itemId, adjustment) => {
        const item = cart.find(cartItem => cartItem.id === itemId);
        if (adjustment === -1 && item.quantity === 1) { // Incorrect: 'item' doesn't exist here
    ...
    return (
        <div>
            ...
            {cart.length === 0 ? (
                ...
                <div className="cart-items-container"> {/* Added Container */} 
                    {cart.map((item) => (
                        <CartWishlistItem 
                            key={item.id} 
                            item={item} 
